...
def get_solar_altaz_multiple_times(times):
    """Solar azimuth and elevation (deg) at OVRO for one or more times."""
    ...
    return azel.az.value, azel.alt.value
...
def slow_pipeline_default_plot(fname, 
    ...
    if spec_fits is None or not os.path.exists(spec_fits):
        ...
        raise NotImplementedError(
            "Spectrogram overlay (spec_fits) is not supported in lwasolarutl."
        )
